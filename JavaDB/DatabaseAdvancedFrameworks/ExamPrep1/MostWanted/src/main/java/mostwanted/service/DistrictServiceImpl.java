...
public class DistrictServiceImpl implements DistrictService {
    ...
    private final static String DISTRICT_JSON_FILE_PATH =
    ...
    public String importDistricts(String districtsFileContent) {
        ...
        DistrictImportDto[] districtImportDtos = this.gson.fromJson(
                districtsFileContent,
                DistrictImportDto[].class);

        Arrays.stream(districtImportDtos)
                .forEach(districtImportDto -> {
                    District districtEntry = this.districtRepository
                    ...
                    districtEntry = this.modelMapper.map(districtImportDto, District.class);
                    districtEntry.setTown(townEntity);
                    this.districtRepository.saveAndFlush(districtEntry);

                    importResult
                            .append(String.format(
                                    Constants.SUCCESSFUL_IMPORT_MESSAGE,
                                    districtEntry.getClass().getSimpleName(),
                                    districtEntry.getName()
                            ))
                            .append(System.lineSeparator());
